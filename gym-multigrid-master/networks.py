...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        expert_K_norm = T.matmul(T.linalg.norm(self.actor_Q(x), dim=-1).unsqueeze(-1), T.linalg.norm(self.expert_K.weight, dim=1).unsqueeze(0)).unsqueeze(-1)# ( b x # rules x 1)
        expert_actions = F.softmax(expert_K_norm * expert_actions, dim=-1) 

        ## Compute weights
        self.w_i = F.cosine_similarity(self.actor_Q(x), self.actor_K(x), dim=1).view(-1,1,1)# (b x1 x1) , Self-attention
        self.W_e = F.cosine_similarity(self.expert_K.weight, self.actor_Q(x).unsqueeze(-2), dim=-1).unsqueeze(-1)# (b x # rules x 1), Expert Cross-Attention
        ...
        x = self.w_i.view(-1,1) * inner_pi + T.sum(self.W_e * expert_actions, dim=-2) #(b x # rules x 1) x (b # rules x # actions)
