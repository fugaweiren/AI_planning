...
class EscapeRoomEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        # Place moving guardians
        self.obstacles = []
        if self.n_guardians <= rWallIdx - lWallIdx:
            self.n_guardians = int(self.n_guardians)
        else:
            self.n_guardians = int((rWallIdx - lWallIdx)/2)
        for i_obst in range(self.n_guardians):
            self.obstacles.append(Ball())
            self.place_obj(self.obstacles[i_obst], top=(lWallIdx, 0), size=(rWallIdx - lWallIdx, height), max_tries=100)
